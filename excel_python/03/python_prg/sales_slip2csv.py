        ...
        for sh in wb:
            for dt_row in range(9,19):
                if sh.cell(dt_row, 2).value != None:
                    ...
                    lsh.cell(list_row, 4).value = sh.cell(7, 8).value   #負責人代碼
                    lsh.cell(list_row, 5).value = sh.cell(dt_row, 1).value #No                    
                    lsh.cell(list_row, 6).value = sh.cell(dt_row, 2).value #商品代碼 
                    lsh.cell(list_row, 7).value = sh.cell(dt_row, 3).value #商品名稱稱
                    lsh.cell(list_row, 8).value = sh.cell(dt_row, 4).value #數量
                    lsh.cell(list_row, 9).value = sh.cell(dt_row, 5).value #單價
                    lsh.cell(list_row, 10).value = sh.cell(dt_row, 4).value * \
                                                sh.cell(dt_row, 5).value #金額
                    lsh.cell(list_row, 11).value = sh.cell(dt_row, 7).value #備註                                    
                    list_row += 1
